#!/usr/bin/env bash
set -euo pipefail

TASK_PY=/root/miniconda3/envs/att-atk/bin/python
TASK_PREFIX=randomselector1000
TASK_SEED=20260907

run_one() {
  local name=$1
  local model=$2
  local batch_size=$3
  local attack_dir="outputs/attack/${TASK_PREFIX}_${name}_adapter_defaults_s1000_seed${TASK_SEED}"
  local csv_path="outputs/csv/outputs_attack_${TASK_PREFIX}_${name}_adapter_defaults_s1000_seed${TASK_SEED}.csv"

  echo "===== ${name}: random-selector attack ====="
  if [[ -d "$attack_dir" ]]; then
    local image_count
    image_count=$(find "$attack_dir" -maxdepth 1 -type f -name 'adv_*.png' | wc -l)
    if [[ "$image_count" -ne 1000 ]] ||
       [[ ! -f "$attack_dir/attack_params.json" ]] ||
       [[ ! -f "$attack_dir/gradient_diagnostics.json" ]] ||
       [[ ! -f "$attack_dir/replay_manifest.json" ]]; then
      echo "Refusing to overwrite incomplete attack directory: $attack_dir" >&2
      return 1
    fi
    echo "Attack already complete: ${name}"
  else
    "$TASK_PY" main.py \
      --attack-method progressive \
      --whitebox-model "$model" \
      --progressive-patch-selector random \
      --batch-size "$batch_size" \
      --max-attacked-samples 1000 \
      --seed "$TASK_SEED" \
      --output-dir "$attack_dir"
  fi

  echo "===== ${name}: 14-target transfer ====="
  if [[ -f "$csv_path" ]]; then
    echo "Transfer CSV already exists: ${name}"
  else
    "$TASK_PY" transfer_eval.py \
      --image-dir "$attack_dir" \
      --amp \
      --exp-name "${TASK_PREFIX}_${name}_adapter_defaults_s1000_seed${TASK_SEED}"
  fi
}

# Checkpoints, drop ratios, score modes, and opponent strengths are deliberately
# omitted: main.py resolves all of them from the source architecture adapter.
run_one vit vit_base_patch16_224 96
run_one cait cait_s24_224 48
run_one pit pit_b_224 96
run_one visformer visformer_small 48

"$TASK_PY" experiments/compare_random_high_asr.py
