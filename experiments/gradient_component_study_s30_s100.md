...
此前的轨迹 probe 只测试了把当前梯度向 MI 历史方向对齐或增强。本轮补充了
`g' = g - α proj_{m_{t-1}}(g)`，并保持 MI 内部存储、衰减和更新完全不变。结果为：

| 正交强度 | Overall Δ | ViT Δ | CNN Δ | 白盒 Δ |
|---:|---:|---:|---:|---:|
| 0.25 | -0.30pp | -0.48pp | 0.00pp | 0.00pp |
| 0.50 | -2.12pp | -1.43pp | -3.33pp | 0.00pp |
| 0.75 | -0.30pp | -1.43pp | +1.67pp | +3.33pp |

因此“MI 历史方向是主要有害源模型成分、去掉它即可提升 ViT”的解释也不成立。当前梯度与历史方向的平行部分包含迁移所需的稳定信号，不能仅凭源模型轨迹几何关系删除。
...
在 196 个候选中，唯一同时通过小样本守门并值得扩大验证的是弱 Gaussian 全量叠加；它已经完成三个 100 样本 seed 的确认，但平均 Overall 只有 `+0.39pp`、ViT `+0.48pp`。因此当前主线必须继续使用原始 `mean`，不能把 Gaussian 或任何其他探针宣称为有效增强。

当前证据已经排除了在“单一 ViT 白盒、固定 20 views/data-aug、只处理聚合梯度”的条件下，基于源梯度自身的静态幅值/频率统计、view 冲突几何、patch 子空间或 MI 历史几何直接获得 `+3–5pp` 的路径。继续运行相同类型的 30 样本遍历没有信息增益；若要重新打开该方向，必须提出新的可证伪信息来源或新的梯度数学结构，并先在小样本上同时通过 ViT/Overall/CNN 守门。

在取得至少一个候选达到 ViT `+3pp`、Overall 同时达到 `+3pp` 且 CNN 不下降超过 `1pp` 之前，不应更新攻击主线。当前报告中的黑盒覆盖仍是项目配置的 11 个模型；“全部可加载 timm 模型”的穷举评估尚未完成，不能将 11 模型结果外推为全 timm 结论。
