"""Trace MI/no-MI gradient history under no-step-projection LM-DSS."""
...
PROTOCOL = "mi_no_step_projection_mechanism_v1"
...
KNOWN_ASR = {
    "projection_on_aug_all_mi": 0.8375,
    "projection_on_background_no_mi": 0.6613,
}
...
def save_trace_payload(output_dir: Path, branch: str, batch_idx: int, payload: dict[str, object]) -> None:
    branch_dir = output_dir / "mi_no_step_projection_trace" / branch
...
def aggregate_report(rows: list[dict[str, object]], args) -> dict[str, object]:
    ...
    report = {
        ...
        "projection_on_reference": {
            "mi_avg_asr": KNOWN_ASR["projection_on_aug_all_mi"],
            "no_mi_avg_asr": KNOWN_ASR["projection_on_background_no_mi"],
            "mi_minus_no_mi_gap": KNOWN_ASR["projection_on_aug_all_mi"] - KNOWN_ASR["projection_on_background_no_mi"],
        },
        "no_step_config": {
            "project_each_step": False,
            "guide_aug_area": "all",
            "pixel_clamp": [0.0, 1.0],
            "epsilon_projection_each_step": False,
            "trace_payload": "compact_no_raw_term_grad_stacks",
        },
        "fft_bands": FFT_BANDS,
...
def build_conclusion_zh(report: dict[str, object]) -> str:
    ...
    mi = source.get("no_step_projection_mi", {})
    no_mi = source.get("no_step_projection_no_mi", {})
    manual = source.get("manual_accumulator_no_mi_flag", {})
    mi_target = target.get("no_step_projection_mi", {})
    lines = [
        "# No-Step-Projection 下 MI 梯度累计机制实验结论",
        "",
        f"- projection-on 参考 100 样本 ASR: MI `{KNOWN_ASR['projection_on_aug_all_mi']:.4f}`，no-MI `{KNOWN_ASR['projection_on_background_no_mi']:.4f}`。",
        ...
        "## 解释",
        "",
        "在 no-step-projection 下，trace 更新与 `attack.py --no-step-projection` 一致：每步只执行 `adv += step_size * sign(update)` 并 clamp 到 `[0, 1]`，不再投影回 epsilon ball。MI 的关键仍是 `momentum_after - current_grad` 这部分历史输入梯度是否保留并参与 `sign(update)`。",
        "no-MI 每一步只使用当前 `grad_after_rotation`，因此所有在前几步出现过、但当前步减弱或反向的历史方向都会从 update 中消失。",
        "如果 `manual_accumulator_no_mi_flag` 与 `no_step_projection_mi` 的指标接近，说明不用 CLI 的 `--mi` 也能实现同样效果；关键机制是跨 step 状态累计，而不是参数名本身。",
    ]
    if manual:
...
def write_outputs(rows: list[dict[str, object]], report: dict[str, object], output_dir: Path) -> None:
    ...
    metrics_path = output_dir / "mi_no_step_projection_mechanism_metrics.csv"
    fieldnames = sorted({key for row in rows for key in row})
    with metrics_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(rows)
    (output_dir / "mi_no_step_projection_mechanism_report.json").write_text(
        json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    (output_dir / "mi_no_step_projection_mechanism_conclusion_zh.md").write_text(
...
def run_experiment(args) -> None:
    ...
    rows: list[dict[str, object]] = []
    branches = (
        ("no_step_projection_mi", True, False),
        ("no_step_projection_no_mi", False, False),
