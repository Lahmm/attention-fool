...
from gradient_study import (
    ...
    CrossScaleCoherentGaussianProbe,
    CrossSamplePrototypeProbe,
...
class GradientProbeTests(unittest.TestCase):
    ...
    def test_raw_temporal_cross_scale_probe_preserves_shape_and_finite_values(self):
        views = torch.randn(20, 2, 3, 16, 16)
        result = RawScaleCrossScaleGaussianProbe(
            0.5, 0.5, 0.5, 4.0, 0.75
        ).apply(views, ["a", "b"], 0)
        self.assertEqual(result.shape, views.shape[1:])
        self.assertTrue(torch.isfinite(result).all())
    ...
    def test_cross_scale_coherent_gaussian_composition(self):
        views = torch.randn(20, 2, 3, 32, 32)
        result = CrossScaleCoherentGaussianProbe(0.5, 0.5, 0.25, 4.0, 0.75).apply(
            views, ["a", "b"], 0
        )
        self.assertEqual(result.shape, views.shape[1:])
        self.assertTrue(torch.isfinite(result).all())

    def test_cross_sample_prototype_is_finite_and_scaled(self):
        views = torch.randn(20, 3, 3, 32, 32)
        probe = CrossSamplePrototypeProbe("low", 4.0, 0.10, ema_decay=0.75)
        result = probe.apply(views, ["a", "b", "c"], 0)
        self.assertEqual(result.shape, views.shape[1:])
        self.assertTrue(torch.isfinite(result).all())
    ...
    def test_orthogonal_gaussian_preserves_shape_and_finite_values(self):
        views = torch.randn(20, 2, 3, 16, 16)
        result = OrthogonalGaussianProbe(4.0, 0.75).apply(views, ["a", "b"], 0)
        self.assertEqual(result.shape, views.shape[1:])
        self.assertTrue(torch.isfinite(result).all())
    ...
    def test_component_probe_names_round_trip(self):
        names = (
            ...
            "raw_temporal_cross_scale_p050_c50_x050_s40_a075",
